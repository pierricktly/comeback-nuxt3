<template>
  <div>
    <section v-if="newsToday.length">
      <div class="relative">
        ...
        >
          ...
          >
            <HomeSlider
              :image="news.artist.images[news.artist.images.length - 1]"
              :name="news.artist.name"
              :id="news.artist.id"
            />
          </SwiperSlide>
        </Swiper>
    ...
    <section
      v-else
      class="relative flex min-h-[20rem] w-full flex-col justify-center overflow-hidden rounded bg-[url('/slider-placeholder.webp')] bg-cover bg-center bg-no-repeat object-cover text-center lg:aspect-video lg:max-h-[36rem]"
    >
      ...
      <div class="z-10 space-y-1.5 xl:space-y-5">
        ...
        <p class="font-semibold lg:text-2xl 2xl:text-3xl">
